...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($carro_id) || empty($mecanico) || empty($data) || empty($descricao)) {
    ...
    $sql = "INSERT INTO agendamentos (usuario_id, carro_id, mecanico, data, descricao, status) 
            VALUES (?, ?, ?, ?, ?, 'agendado')";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iisss", $usuario_id, $carro_id, $mecanico, $data, $descricao);
